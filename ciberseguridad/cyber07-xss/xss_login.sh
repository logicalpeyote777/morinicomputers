#!/usr/bin/env bash
# xss_login.sh -- inicia sesion en MI DVWA del laboratorio aislado y deja lista la cookie
# de sesion para los ataques de XSS y CSRF.
#
#   source xss_login.sh [nivel]      # nivel = low (def) | impossible
#
# Al cargarlo con "source" exporta en tu shell:
#   COOKIE -> "PHPSESSID=...; security=<nivel>"   (lista para curl)
#   XSSR   -> URL del XSS Reflejado     (/vulnerabilities/xss_r/)
#   XSSS   -> URL del XSS Almacenado    (/vulnerabilities/xss_s/)
#   CSRF   -> URL del cambio de clave   (/vulnerabilities/csrf/)
...
# 3) fijo el nivel de seguridad de DVWA (low = XSS y CSRF posibles)
...
# 4) compongo la cookie (PHPSESSID + security) y exporto cookie + URLs vulnerables
SID=$(awk '/PHPSESSID/{print $7}' "$JAR" | tail -1)
rm -f "$JAR"
export COOKIE="PHPSESSID=$SID; security=$LEVEL"
export XSSR="$URL/vulnerabilities/xss_r/"
export XSSS="$URL/vulnerabilities/xss_s/"
export CSRF="$URL/vulnerabilities/csrf/"
echo "COOKIE = $COOKIE"
echo "XSSR   = $XSSR   (reflejado)"
echo "XSSS   = $XSSS   (almacenado)"
echo "CSRF   = $CSRF   (cambio de clave)"
